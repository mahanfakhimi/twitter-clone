import VideoPlayer from "../VideoPlayer";
import TweetInteractions from "./TweetInteractions";
import TweetPopover from "./TweetPopover";

const Tweet = () => {
  return (
    <div className="flex gap-x-4 p-4 border-b border-b-neutral-200 dark:border-b-neutral-800">
      <img
        src="data:image/jpeg;base64,"
        alt=""
        className="w-12 h-12 object-cover rounded-full"
      />
      <div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-x-1">
            <h3 className="dark:text-white font-semibold hover:underline">PlayStation</h3>
            <span className="text-neutral-500 text-xs">@PlayStation · 23h</span>
          </div>
          <TweetPopover />
        </div>

        <div>
          <span className="text-neutral-500">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Libero, dicta animi? Delectus reiciendis eveniet aspernatur neque! Ratione dolorem id incidunt neque asperiores quidem at cum.&nbsp;</span>
          {["React", "Node", "Express", "Nuxt", "Next", "Nest", "Bun", "Deno", "Vue", "Angular", "Developer"].map((tag, index) => (
            <span className="inline-block text-sky-500 cursor-pointer hover:underline" key={index}>
              #{tag}&nbsp;
            </span>
          ))}
        </div>

        <VideoPlayer />
        <TweetInteractions />
      </div>
    </div>
  );
};

export default Tweet;
